...
import { Facebook, Instagram, Mail, MapPin, Phone, Sparkles, Truck, Youtube } from 'lucide-react';
import { useStore } from '../context/StoreContext';

export const Footer: React.FC = () => {
  const { siteSettings, setCurrentView, setIsTrackOrderOpen } = useStore();
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-12">
        <div className="bg-gradient-to-r from-rose-900/60 via-purple-900/60 to-slate-900 p-8 rounded-3xl border border-rose-500/20 shadow-2xl flex flex-col lg:flex-row items-center justify-between gap-6">
          <div>
            ...
            <h3 className="text-2xl font-black text-white font-serif">Get Rs 500 OFF On Your First Order</h3>
            <p className="text-xs text-slate-300 mt-1">
              Subscribe to receive secret flash sales, festive coupon codes and parenting fashion guides.
            </p>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-8 text-xs">
          <div className="lg:col-span-2 space-y-4">
            ...
            <p className="text-slate-400 leading-relaxed max-w-sm">
              India&apos;s premier modern kids fashion destination. Crafting 100% GOTS certified organic, skin-safe,
              hypoallergenic apparel for babies and toddlers.
            </p>
          ...
          <div className="space-y-3">
            ...
            <ul className="space-y-2 text-slate-400">
              <li>
                <button onClick={() => setCurrentView('home')} className="hover:text-rose-400">
                  Home
                </button>
              </li>
              <li>
                <button onClick={() => setCurrentView('shop')} className="hover:text-rose-400">
                  All Collections
                </button>
              </li>
              <li>
          ...
          <div className="space-y-3">
            ...
            <ul className="space-y-2 text-slate-400">
              <li>
                ...
                >
                  <Truck className="w-3.5 h-3.5" /> Track Order Status
                </button>
              </li>
              <li>
                <button onClick={() => setCurrentView('about')} className="hover:text-rose-400">
                  About Our Brand
                </button>
              </li>
              <li>
                <button onClick={() => setCurrentView('blog')} className="hover:text-rose-400">
                  Parenting Blog
                </button>
              </li>
            </ul>
        ...
        <div className="pt-8 border-t border-slate-800 flex flex-col sm:flex-row items-center justify-between text-xs text-slate-500 gap-4">
          <p>Copyright 2026 Akshvik Tiny Trends. All Rights Reserved.</p>
          <div className="flex items-center gap-3 text-[10px] uppercase tracking-wider font-mono">
            ...
            <span>&bull;</span>
            <span>UPI and Card Accepted</span>
          </div>
        </div>
      </div>
    </footer>
